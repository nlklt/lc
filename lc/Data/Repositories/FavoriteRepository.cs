...
public sealed class FavoriteRepository : IFavoriteRepository
{
    ...
    public async Task AddAsync(int userId, int bookId)
    {
        ...
        var exists = await _db.Favorites.AnyAsync(x => x.UserId == userId && x.BookId == bookId);
        if (exists)
            return;
    ...
    {
        return await _db.Favorites.AnyAsync(x => x.UserId == userId && x.BookId == bookId);
    }

    public async Task<IReadOnlyList<BookListItemDto>> GetByUserIdAsync(int userId)
    {
